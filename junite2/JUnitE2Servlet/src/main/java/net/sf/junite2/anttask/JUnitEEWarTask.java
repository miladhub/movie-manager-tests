...
public class JUnitEEWarTask extends War{
	...
	private List<FileSet> testCases = new LinkedList<FileSet>();
	private List<ZipFileSet> classes = new LinkedList<ZipFileSet>();
	private List<EjbRef> ejbRefs = new LinkedList<EjbRef>();
	private List<EjbLocalRef> ejbLocalRefs = new LinkedList<EjbLocalRef>();
	private List<ResRef> resRefs = new LinkedList<ResRef>();
	 ...
	 * This parameter is just the name of a .jar file (without any directory) (eg.: test.jar) If this
	 * parameter is not set, the classes will appear as standalone classes in WEB-INF/classes If this
	 * parameter is set, the classes will be bundled into a jar file stored in
	 * WEB-INF/lib/<testjarname>
	 */
	...
	/**
	 * The nested testCases element specifies a FileSet. All files included in this fileset will be
	 * considered as the test classes to run. Those classes will appear in the generated index.html
	 * and the user will be able to run them individually
	...
	/**
	 * Create a nested ejbRef element. An EjbRef describes a <ejb-ref> in the generated web.xml file.
	...
	/**
	 * Create a nested ejbLocalRef element. An EjbLocalRef describes a <ejb-local-ref> in the
	...
	/**
	 * Check that all the required parameters have been properly set. A BuildException is thrown if
	 * the task is not properly configured
	 */
		...
		try{
			...
			for(FileSet fs : testCases){
				...
				for(int j = 0; j < files.length; j++){
					...
					bufferList.append(className).append("\">&nbsp;&nbsp;").append(className).append(
						"</td></tr>\n");
				}
			}
	 ...
	 */
	public static String getTestCaseClassName(String fileName) throws BuildException{
