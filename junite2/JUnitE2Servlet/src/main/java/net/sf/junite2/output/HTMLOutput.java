...
public class HTMLOutput extends AbstractOutput{
	...
	/**
   */
	public HTMLOutput(TestRunnerResults results, HttpServletRequest request,
			HttpServletResponse response, boolean filterTrace, int refreshDelay) throws IOException{
	...
	protected void printHeader(){
		...
		if(!isFinished()){
			...
			pw.println("<META HTTP-EQUIV=\"Refresh\" CONTENT=\"" + refreshDelay + "; URL="
				+ response.encodeURL(redirect) + "\">");
		}
		pw.println("</head>");
	...
	protected void printUnderProgress(){
		...
		if(currentTest != null){
			pw.println("<tr><td class=\"failedcell\">Current test: " + currentTest.getTestName()
				+ "</td></tr>");
		}
		if(isStopped() && !isFinished()){
			...
			pw
				.print("<tr><td class=\"failedcell\"><input type=\"submit\" name=\"stop\" value=\"Stop execution\"></td>");
	...
	protected void printSummary(boolean createInfoAndLinks){
		...
		while(suites.hasNext()){
			...
			if(createInfoAndLinks){
				...
				if(suite.successful()){
					pw.println("<tr><td class=\"passedcell\">" + image(RESOURCE_GREEN_BULLET, "Success")
						+ "</td>");
				}else if(suite.hasError()){
					pw.println("<tr><td class=\"failedcell\">" + image(RESOURCE_RED_BULLET, "Error")
						+ "</td>");
				}else{
					pw.println("<tr><td class=\"failedcell\">" + image(RESOURCE_YELLOW_BULLET, "Failure")
						+ "</td>");
				}
			}
	...
	protected void printMethodList(){
		...
		while(suites.hasNext()){
			...
			while(tests.hasNext()){
				...
				pw.println("</td>");
				pw.println("<td width=\"100%\" class=\"cell\">" + test
					+ "</td><td class=\"cell\" align=\"right\">");
	...
	private String image(String resource, String alt){
		...
		buffer.append("<img src=\"").append(response.encodeURL(url.toString())).append(
			"\" border=\"0\" height=\"14\" widht=\"14\" alt=\"");
	...
	private String singleTestLink(TestInfo test, String text){
		...
		buffer.append("<a href=\"").append(response.encodeURL(url.toString())).append("\">").append(
			text).append("</a>");
		return buffer.toString();
	}
