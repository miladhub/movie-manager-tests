...
import net.sf.junite2.output.HTMLOutput;
import net.sf.junite2.output.OutputProducer;
import net.sf.junite2.output.XMLOutput;
import net.sf.junite2.runner.TestRunner;
 ...
 * can change this behavior by subclassing this class and overwrite the method
 * {@link #getDynamicClassLoader} to answer the classloader of your choice.
 * @author <a href="mailto:oliver@oross.net">Oliver Rossmueller</a>
 */
public class JUnitEEServlet extends HttpServlet{

	private static final long serialVersionUID = 3995082876898080847L;

	/**
	 * The form parameter which defines the name of the suite class to run. This parameter can appear
	 * more than once to run multiple test suites.
	 */
	...
	/**
	 * The form parameter which defines if resources should be checked to run all included test cases
	 */
	...
	/**
	 * Answer the classloader used to load the test classes. The default implementation answers the
	 * classloader of this class, which usally will be the classloader of the web application the
	 * servlet is a part of. <p/> If this default behaviour does not work for you, overwrite this
	 * method and answer the classloader that fits your needs.
	 */
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(testClassNames == null){
			...
			errorResponse(searchForTests(request.getParameterValues(PARAM_SEARCH)), request
				.getContextPath()
				+ request.getServletPath(), message, output, request, response, xsl, filterTrace,
				showMethods);
			return;
		}
		...
		System.setProperty(CACTUS_CONTEXT_URL_PROPERTY, "http://" + request.getServerName() + ":"
	...
	protected void renderResults(TestRunnerResults results, HttpServletRequest request,
			HttpServletResponse response, String xsl, boolean filterTrace) throws IOException{
		OutputProducer output = getOutputProducer(results, request.getParameter(PARAM_OUTPUT), request,
			response, xsl, filterTrace);
	...
	protected TestRunnerResults runTests(String test, String[] testClassNames,
			HttpServletRequest request, boolean forkThread){
	...
	/**
	 * Answer the default output format of the test report. This implementation returns html as the
	 ...
	 * @throws IOException
	 */
	protected OutputProducer getOutputProducer(TestRunnerResults results, String outputParam,
			HttpServletRequest request, HttpServletResponse response, String xsl, boolean filterTrace)
			throws IOException{
	...
	protected void errorResponse(String[] testCases, String servletPath, String message,
			String output, HttpServletRequest request, HttpServletResponse response, String xsl,
			boolean filterTrace, boolean showMethods) throws IOException{
	...
	protected void printIndexHtml(String[] testCases, String servletPath, String message,
			PrintWriter pw, boolean showMethods) throws IOException{
		InputStream in = getClass().getClassLoader().getResourceAsStream(
			RESOURCE_PREFIX + "/runner.html");
		...
		if(testCases != null){
			...
			for(int i = 0; i < testCases.length; i++){
				...
				bufferList.append(testCases[i]).append("\">&nbsp;&nbsp;").append(testCases[i]).append(
					"</td></tr>\n");
	 ...
	 * @param servletPath
	 */
	protected void printIndexHtmlTestMethods(StringBuffer bufferList, String testCase,
			String servletPath){
		...
		if(methods.length > 0){
			...
			for(int j = 0; j < methods.length; j++){
				...
				bufferList.append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\""
					+ servletPath + "?" + PARAM_SUITE + "=" + testCase + "&" + PARAM_TEST + "=" + methods[j]
					+ "\">");
	...
	/**
	 * Looks up all the test methods for a particular test class. Not particularly efficient, it just
	 * leverages existing test lookup functionality from {@link TestRunner}.
	 */
	protected String[] getTestClassMethods(String testClass){
		List<String> testMethodList = new LinkedList<String>();
		Class<?> clazz = null;

		try{
			clazz = this.getClass().getClassLoader().loadClass(testClass);
