/*
 * $Id: JUnitEETask.java,v 1.1.1.1 2007/07/13 23:45:13 martinfr62 Exp $ (c) 2002 Oliver Rossmueller
 */
...
public class JUnitEETask extends Task{
	...
	private List<JUnitEETest> tests = new LinkedList<JUnitEETest>();
	...
	/**
	 * Tell the task how to handle test failures. If set to true, the task will stop execution if a
	 * test failure or error occurs. If set to false, the task will continue execution.
	...
	private boolean parseResult(InputStream in, JUnitEETest test) throws Exception{
		...
		List<JUnitEEResultFormatter> resultFormatters = createFormatters(test);
	...
	private List<JUnitEEResultFormatter> createFormatters(JUnitEETest test) throws Exception{
		List<JUnitEEResultFormatter> answer = new LinkedList<JUnitEEResultFormatter>();
		...
		for(Enumeration<?> e = test.getFormatters(); e.hasMoreElements();){
			FormatterElement element = (FormatterElement)e.nextElement();
